...
if [[ -n "$platform" ]]; then
  curl_args+=(--data-urlencode "platform=${platform}")
fi
if [[ -n "$scene_type" ]]; then
  curl_args+=(--data-urlencode "sceneType=${scene_type}")
fi
if [[ -n "$intention_level" ]]; then
  curl_args+=(--data-urlencode "intentionLevel=${intention_level}")
fi
if [[ -n "$follow_status" ]]; then
  curl_args+=(--data-urlencode "followStatus=${follow_status}")
fi
